---
description: 'Agent to create a technical modernization plan for Java and .NET projects. Auto-detects project type and creates plans for Java 8→17+ (async, Spring Boot 3.x) or .NET legacy→.NET 8+ (async/await, minimal APIs, IOptions<T>). Integrates SonarQube findings.'
...
# Task: Create Modernization Plan for Java & .NET Projects
...
1. Scan the workspace for `pom.xml` or `build.gradle` (Java) vs `*.sln` or `*.csproj` (.NET)
2. Set `PROJECT_TYPE`, `TARGET_VERSION`, and `TECH_STACK`:
   - Java: `Java 17+`, `Spring Boot 3.x / Jakarta EE`
   - .NET: `.NET 8+`, `ASP.NET Core / Minimal APIs`
3. Read `.github/instructions/migration-plan-template.instructions.md` completely
4. Understand the required structure, formatting, and restrictions
...
1. **Pre-Planning Validation**:
   ...
   - **Check if Migration/01-Migration_Plan.md already exists**:
     ...
     - If user declines regeneration, proceed directly to handoff to `modernization-developer` agent
...
1. **Architecture Analysis** → Create subagent with this prompt:
   ```
   Analyze the current legacy application architecture and identify:
   1. Current patterns (synchronous operations, configuration approaches, architecture)
   2. Target patterns:
      - Java: Java 17+ async (CompletableFuture/reactive), Spring Boot 3.x config, modular design
      - .NET: async/await, IOptions<T> config, minimal APIs, modular design
...
4. **Handoff to Developer Agent**:
   After successful plan creation and validation, **automatically hand off** to the `modernization-developer` agent.
...
### ✅ REQUIRED Action:
The handoff to `modernization-developer` agent is **configured in the frontmatter** (lines 12-16) and is **automatic**.
