---
description: 'Agent to create a technical modernization plan for migrating .NET Framework to .NET 6+ with async/await, externalized config, modularization, and modern APIs'
...
handoffs: 
  - label: Execute modernization
    agent: dotnet-modernization-developer
    prompt: Execute the .NET Framework to .NET 6+ modernization following the comprehensive plan in Migration/01-Migration_Plan.md. Proceed through all phases automatically without stopping.
    send: true
---

# Task: Create Modernization Plan for .NET Framework to .NET 6+
...
1. **Pre-Planning Validation**:
   ...
   - **Check if Migration/01-Migration_Plan.md already exists**:
     ...
     - If user declines regeneration, proceed directly to handoff to `dotnet-modernization-developer` agent
...
1. **Architecture Analysis** → Create subagent with this prompt:
   ```
   Analyze the current .NET Framework application architecture and identify:
...
When dealing with large .NET Framework applications or complex architectures, use these strategies to manage scope and avoid token/length limits:
...
If the .NET Framework application has many controllers, services, or components:
...
4. **Handoff to Developer Agent**:
   After successful plan creation and validation, **automatically hand off** to the `dotnet-modernization-developer` agent.
...
You are creating the Task List section for a .NET Framework to .NET 6+ modernization plan.
...
- Focus on .NET Framework → .NET 6+ transformation
...
### ✅ REQUIRED Action:
The handoff to `dotnet-modernization-developer` agent is **configured in the frontmatter** (lines 12-16) and is **automatic**.
