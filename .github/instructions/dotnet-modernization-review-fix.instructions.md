...
# .NET Framework to .NET 6+ Modernization Review Fix Instructions

## Purpose
Guide the remediation of issues identified during the modernization review process. Use this document when addressing findings from `dotnet-modernization-review.instructions.md`.
...
### Issue: Blocking Calls (`.Result`, `.Wait()`)

**Problem**: Code uses `.Result` or `.Wait()` causing potential deadlocks
```csharp
// ❌ BLOCKING
...
**Fix**: Replace with `await` and make method `async`
```csharp
// ✅ FIXED
...
2. Change return type to `Task<T>` or `Task`
3. Replace `.Result` with `await`
4. Replace `.Wait()` with `await`
